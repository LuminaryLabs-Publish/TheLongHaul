...
assert.match(combined, /vegetation=visualOnly\?patch\.vegetation\.filter\(\(_,index\)=>index%6===0\)/);
assert.match(combined, /if\(!options\.visualOnly\)for\(const item of patch\.scenery\?\?\[\]\)_lhAddRoadside/);
...
assert.match(combined, /setCellStreamingTargets\(\[\.\.\.preloadedRouteCells,\.\.\.desired,\.\.\.frontier\]\)/);
assert.match(combined, /const routeCells=createRouteCells\(course\)/);
assert.match(combined, /reconcileCells\(routeCells\.length\+windowSize\)/);
...
assert.match(combined, /targetFar=clamp\(590-weather\*70-wave\*20-speed\*18,500,610\)/);
assert.match(combined, /cacheLimit:104/);
