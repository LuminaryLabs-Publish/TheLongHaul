...
export const DEFAULT_LONG_HAUL_WORLD_PROFILE = Object.freeze({
  id: "massive-country",
  worldSeed: "long-haul",
  playableRadius: 22000,
  boundaryFadeWidth: 2200,
...
function normalizeProfile(input = {}) {
  ...
  return {
    ...
    playableRadius: Math.max(2000, Number(input.playableRadius ?? DEFAULT_LONG_HAUL_WORLD_PROFILE.playableRadius)),
    boundaryFadeWidth: Math.max(100, Number(input.boundaryFadeWidth ?? DEFAULT_LONG_HAUL_WORLD_PROFILE.boundaryFadeWidth)),
...
export function createLongHaulWorldProfileKit(N, options = {}) {
  ...
  const kit = N.defineDomainServiceKit({
    ...
    services: ["world-profile", "disk-policy", "lod-policy"],
